...
public class OfferSubscriptionRepositoryTest : IAssemblyFixture<TestDbFixture>
{
    ...
    public async Task GetAllBusinessAppDataForUserIdAsync_WithValidUser_ReturnsExpectedResult()
    {
        ...
        // Assert
        result.Should().ContainSingle().Which.Should().Match<(Guid OfferId, Guid SubscriptionId, string? OfferName, string SubscriptionUrl, Guid LeadPictureId, string Provider, string? ProviderShortName)>(x =>
    ...
    public async Task GetOfferSubscriptionDetailForProviderAsync_WithNotExistingId_ReturnsExpected()
    {
        ...
        // Act
        var result = await sut.GetOfferSubscriptionDetailsForProviderAsync(Guid.NewGuid(), new Guid("3DE6A31F-A5D1-4F60-AA3A-4B1A769BECBF"), _userCompanyId, OfferTypeId.SERVICE, new List<Guid>());
