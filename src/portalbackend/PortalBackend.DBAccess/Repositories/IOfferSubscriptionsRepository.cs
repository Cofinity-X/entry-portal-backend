...
public interface IOfferSubscriptionsRepository
{
    ...
    IAsyncEnumerable<(Guid OfferId, Guid SubscriptionId, string? OfferName, string SubscriptionUrl, Guid LeadPictureId, string Provider, string? ProviderShortName)> GetAllBusinessAppDataForUserIdAsync(Guid userId);
