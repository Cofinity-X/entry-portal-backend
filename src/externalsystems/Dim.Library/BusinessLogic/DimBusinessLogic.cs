...
public class DimBusinessLogic : IDimBusinessLogic
{
    ...
    public async Task ProcessDimResponse(string bpn, DimWalletData data, CancellationToken cancellationToken)
    {
        ...
        _portalRepositories.GetInstance<ICompanyRepository>().CreateWalletData(companyId, data.Did, data.DidDocument, data.AuthenticationDetails.ClientId, secret, initializationVector, _settings.EncryptionConfigIndex, data.AuthenticationDetails.AuthenticationServiceUrl, data.AuthenticationDetails.IsWalletCustomerProvider);
